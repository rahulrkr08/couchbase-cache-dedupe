...
class CouchbaseStorage {
  constructor (options = {}) {
    ...
    // Couchbase has a key length limit of 250 bytes
    // Keys longer than 200 bytes (leaving room for prefix) will be automatically hashed
    this.maxKeyLength = 200
  }

  /**
   * Generate a hash for a key if it exceeds the max length
   * Keys are automatically hashed using SHA-256 to handle Couchbase's 250-byte key limit
   * @private
   */
  _hashKey (key) {
    // If key is within limit, return as-is for better readability
    if (key.length <= this.maxKeyLength) {
      return key
    }

    // Generate SHA-256 hash for long keys
    // Use hex encoding for readability in Couchbase UI
   ...
   * @returns {Promise<Object|undefined>} The cached value or undefined if not found
   */
  async get (key) {
    try {
  ...
  async set (key, value, ttl, references) {
    ...
    await this.collection.upsert(valueKey, value, { expiry })
  ...
  async refresh (key) {
    try {
      ...
      if (result.expiry) {
        ...
        const now = Date.now()
        ttl = Math.floor((expiryTime - now) / 1000)
        ttl = ttl > 0 ? ttl : 0
      }

      // Touch the document to refresh its expiry
      await this.collection.touch(this._getValueKey(key), ttl)
    } catch (error) {
      if (error.name === 'DocumentNotFoundError') {
        return
      }
  ...
  _getValueKey (key) {
    ...
    return `${this.valuePrefix}${hashedKey}`
  }
  ...
  _getReferenceKey (reference) {
    const hashedRef = this._hashKey(reference)
    return `${this.referencesPrefix}${hashedRef}`
  }
}
